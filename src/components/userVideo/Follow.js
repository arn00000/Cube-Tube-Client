...
export const Follow = ({ id }) => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery(["userId", id], () => getUserById(id));
    ...
    <>
      {decoded.data._id === id ? null : user.follow.find(
        ...
        <button
          ...
          className="sm:text-base text-xs py-2 px-2 sm:px-5 bg-orange-400 text-white rounded-lg"
        >
        ...
        <button
          ...
          className="sm:text-base text-xs py-2 px-2 sm:px-5 uppercase bg-orange-600 text-white rounded-lg"
        >
          Follow
        </button>
      )}
